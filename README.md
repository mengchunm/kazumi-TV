...
如需自动同步上游发布并构建 Android TV 包的版本，可在独立仓库 `kazumi-TV` 中启用 `.github/workflows/sync-tv-release.yaml` 工作流。该工作流会定时检查 `Predidit/Kazumi` 最新 release，若发现新版本且当前仓库尚未发布对应 `tv-<tag>`，则自动构建并发布：
- `armeabi-v7a`
- `arm64-v8a`

启用前需要在仓库 Secrets 中配置：
- `SIGNING_KEY_BASE64`
- `KEY_ALIAS`
- `KEY_STORE_PASSWORD`
- 可选：`DANDANAPI_APPID`
